...
func (renderer *PxCanvasRenderer) Refresh() {
	if renderer.pxCanvas.reloadImgage {
		...
		renderer.pxCanvas.reloadImgage = false
	}
...
func (renderer *PxCanvasRenderer) LayoutCanvas(size fyne.Size) {
	...
	renderer.canvasImage.Move(fyne.NewPos(float32(renderer.pxCanvas.CanvasOffset.X), float32(renderer.pxCanvas.CanvasOffset.Y)))
